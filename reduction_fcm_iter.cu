...
//         // cudaMemcpy(device_in_num, d_membership, in_size * sizeof(float), cudaMemcpyDeviceToDevice);
//         // cudaMemcpy(device_in_den, d_membership, in_size * sizeof(float), cudaMemcpyDeviceToDevice);
//         cudaMemcpy(device_in_num, d_membership, in_size * sizeof(float), cudaMemcpyHostToDevice);
//         cudaMemcpy(device_in_den, d_membership, in_size * sizeof(float), cudaMemcpyHostToDevice);

//         for (int j = 0; j < i_num_clutsers; ++j) {
//             blks_c = 1 + (rows * cols - 1) / threads_per_block;
//             while (true) {
//                 update_centers_numerator_kernel<<<blks_c, threads_per_block, shared_size_c>>>(d_image, device_in_num, d_cluster_centers, rows, cols, i_num_clutsers, i_m, device_out_num, j);
//                 update_centers_denominator_kernel<<<blks_c, threads_per_block, shared_size_c>>>(d_image, device_in_den, d_cluster_centers, rows, cols, i_num_clutsers, i_m, device_out_den, j);
...
__host__ void fcm_step(float *i_image, float *i_membership, float *i_cluster_centers, int rows, int cols, int T, int i_num_clutsers, int i_m, int threads_per_block, float *out_membership) {
    ...
    for (int i = 0; i < T; ++i) {
        ...
        for (int j = 0; j < i_num_clutsers; ++j) {
            blks_c = 1 + (rows * cols - 1) / threads_per_block;
            while (true) {
                update_centers_numerator_kernel<<<blks_c, threads_per_block, shared_size_c>>>(d_image, d_membership, d_cluster_centers, rows, cols, i_num_clutsers, i_m, device_out_num, j);
                update_centers_denominator_kernel<<<blks_c, threads_per_block, shared_size_c>>>(d_image, d_membership, d_cluster_centers, rows, cols, i_num_clutsers, i_m, device_out_den, j);
